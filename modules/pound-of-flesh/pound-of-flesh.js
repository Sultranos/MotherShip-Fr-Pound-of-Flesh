...
function verifyPOFHooksStatus() {
  ...
  // Vérification simplifiée - on vérifie seulement que nos managers sont initialisés
  const managersOk = game.poundOfFlesh && 
                    game.poundOfFlesh.cyberwareManager && 
                    game.poundOfFlesh.actorMods &&
                    game.poundOfFlesh.itemMods;
  
  if (managersOk) {
    console.log('Pound of Flesh | All managers are properly initialized');
    return true;
  } else {
    console.warn('Pound of Flesh | Some managers are missing - attempting restoration');
    // Tentative de restauration manuelle
    if (typeof CyberwareManager !== 'undefined' && 
        typeof ActorSheetModifications !== 'undefined' && 
        typeof ItemSheetModifications !== 'undefined') {
      
      if (!game.poundOfFlesh) game.poundOfFlesh = {};
      
      game.poundOfFlesh.cyberwareManager = game.poundOfFlesh.cyberwareManager || new CyberwareManager();
      game.poundOfFlesh.actorMods = game.poundOfFlesh.actorMods || new ActorSheetModifications();
      game.poundOfFlesh.itemMods = game.poundOfFlesh.itemMods || new ItemSheetModifications();
      ...
      console.log('Pound of Flesh | Manual restoration successful');
      return true;
    } else {
      console.error('Pound of Flesh | Cannot restore - classes not available');
      return false;
    }
  }
}
